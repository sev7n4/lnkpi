...
function buildMergeUserMessage(entries: MergeTextSource[], downstreamType: keyof typeof DOWNSTREAM_SYSTEM): string {
  const blocks = entries.map((e) => `【${e.refKey}·${e.label}】\n${e.text}`).join('\n\n')
  return `下游类型：${downstreamType}\n\n请将以下文本参考归纳成一条可直接用于生成的内容：\n\n${blocks}`
}
...
export async function mergeRefsToPrompt(input: {
  ...
  const res = await fetch(`${baseUrl}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      temperature: 0.3,
      messages: [
        { role: 'system', content: DOWNSTREAM_SYSTEM[input.downstreamType] },
        { role: 'user', content: buildMergeUserMessage(entries, input.downstreamType) },
      ],
    }),
  })
  if (!res.ok) {
