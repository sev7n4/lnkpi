...
| 文字 / 贴纸叠加 | **暂不做**：改变产品形态（图层模型），远期再议 | — | 远期 |
| 批量处理（多图套用同一工具） | **依赖项**：等批量基础设施（agent 批量已有雏形），暂不排期 | — | 待依赖就绪 |
| 历史记录面板（多步撤销可视化） | 已有版本条（VersionStrip）覆盖核心场景，暂不升级 | — | 待观察 |

### 14.4 排序结论（下一期打样范围）

1. **P0**：宫格切分迁 Shell（规范验证）＋ 一键抠图（免费获客，rembg 自托管）。
2. **P1**：裁剪 + 旋转翻转（确定性变换 `dock: null` 形态打样）＋ 调色（同期评估，同为确定性变换）。
3. **P2**：局部重绘 / 消除替换（选区子工具复用变现）→ 抠主体 / 擦除 → 超分 / 增强。
4. 每个新包的验收 = 本 spec §12 验收清单 + 注册表三件套完整性审查；信息架构不再逐包重新设计，只回答 §4 的三个判据问题（粒度？产物类型？焦点落点？）。
