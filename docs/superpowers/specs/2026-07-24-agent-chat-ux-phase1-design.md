...
| **同画布再开一轮营销** | **追加拆图**（再生成一套下游骨架），不原地替换第一轮节点；方案节点可更新文案 |
| **生产 SSE 代理** | Vercel `/api/proxy` 对 `agent/chat/conversation` 必须长超时、禁重试、透传流；禁止 20s 缓冲重试 |
...
7. **（2026-07-24 脏画布复测）** 二次确认后服务端曾出现约 20 节点，但前端只见第一套；侧栏常停在「上一轮仍在处理中」。根因：Vercel proxy 默认 **20s 超时 + 最多 3 次重试** 对非幂等 SSE 再打一枪「确认」，且 `arrayBuffer` 缓冲导致进度/canvas_action 到不了浏览器；本地 `saveCanvas` 再用旧图覆盖 Nest 追加节点。
...
8. **同画布第二轮营销确认** → **追加**下游骨架（节点数增加），不删除第一轮节点；流结束后画布应从服务端回拉可见新节点。  
9. **生产经 Vercel proxy**：确认拆图 SSE 可持续超过 20s 且**不会**因代理重试再写入第二条 user「确认」；侧栏应见到进度而非长期卡在 busy tip。  
10. **首轮出图后修改**（复测必覆盖）：点「要修改」或说明调整 → `revise` 更新同一「营销方案」节点并再进确认门；确认后行为符合 §0「追加拆图」或后续产品决议。
...
- **对话内结构化任务清单 / 自动重试 / 自动出视频** → `2026-07-25-agent-task-progress-card-design.md`（待审阅）。
...
| 2026-07-25 | 锁定「同画布追加拆图」；记录 Vercel proxy 20s 重试根因与修复约束；补验收 8–10；任务清单 UX 外置 |
